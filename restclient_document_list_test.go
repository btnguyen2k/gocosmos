...
func _verifyChangeFeed(f funcTestFatal, testName string, result *RespListDocs, maxItemCounts, expectedNumDocs int) {
	if result.Error() != nil {
...
func _fetchChangeFeedAndVerify(f funcTestFatal, testName string, client *RestClient, req ListDocsReq, expectedNumDocs int) *RespListDocs {
...
func _testRestClientGetChangeFeed(t *testing.T, testName string, client *RestClient, dbname, collname string) {
	testCases := []struct {
		name            string
		maxItemCount    int
		expectedNumFeed int
	...
	req := ListDocsReq{DbName: dbname, CollName: collname, IsIncrementalFeed: true, MaxItemCount: -1}
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			for _, doc := range dataList {
				...
				doc["touch"] = now
				resp := client.ReplaceDocument(doc.Etag(), DocumentSpec{DbName: dbname, CollName: collname, PartitionKeyValues: []interface{}{username}, DocumentData: doc})
				if resp.Error() != nil {
			...
			for i := len(dataList) - 1; i >= 0; i-- {
				...
				resp := client.ReplaceDocument(dataList[i].Etag(), DocumentSpec{DbName: dbname, CollName: collname, PartitionKeyValues: []interface{}{username}, DocumentData: dataList[i]})
				if resp.Error() != nil {
					t.Fatalf("%s failed: %s", testName+"/ReplaceDocument", resp.Error())
				}
			}
...
func _testRestClientListDocuments(t *testing.T, testName string, client *RestClient, dbname, collname string) {
	testCases := []struct {
		name            string
		maxItemCount    int
		expectedNumFeed int
	...
	req := ListDocsReq{DbName: dbname, CollName: collname, MaxItemCount: -1}
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			if testCase.maxItemCount > 0 {
				req.MaxItemCount = testCase.maxItemCount
