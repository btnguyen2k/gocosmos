...
func TestStmtUpdate_Exec_DefaultDb(t *testing.T) {
	...
	testData := []struct {
	...
	}{
		{
			...
			initParams:   [][]interface{}{nil, nil, nil, nil, {"1", "user", "user@domain.com", 1, true, "user"}, {"2", "user", "user2@domain.com", 1, true, "user"}},
			sql:          `UPDATE tbltemp SET grade=2.0,active=false,data="\"a string 'with' \\\"quote\\\"\"" WHERE id=1`,
			args:         []interface{}{"user"},
			affectedRows: 1,
		},
		{
		...
		{
			name:         "error_uk",
			sql:          `UPDATE tbltemp SET email="\"user2@domain.com\"" WHERE id=1`,
			args:         []interface{}{"user"},
			mustConflict: true,
		},
		{
		...
		},
		{
			...
			initParams:   [][]interface{}{nil, nil, nil, nil, {"1", "user", "user@domain.com", 1, true, "user"}, {"2", "user", "user2@domain.com", 1, true, "user"}},
			sql:          `UPDATE tbltemp SET grade=:1,active=@2,data=$3 WHERE id=:4`,
			args:         []interface{}{2.0, false, "a string 'with' \"quote\"", "1", "user"},
			affectedRows: 1,
		},
		{
